...
const TYPES = [
    ...
    { n: "٤", en: "Social Anxiety", ar: "القلق الاجتماعي", color: "#7571b8", pts: ["رعب حقيقي من التواجد تحت مجهر الآخرين — ليس مجرد خجل، بل خوف عميق ومستمر من التقييم السلبي أو الرفض.", "يسيطر على الفرد هاجس بأنه سيتصرف بطريقة محرجة أو مهينة.", "يدفعه ذلك للانسحاب من التفاعلات الاجتماعية أو تحملها بضغط نفسي هائل يتجاوز التقييم العقلاني للموقف."] },
    { n: "٥", en: "Separation Anxiety", ar: "قلق الانفصال", color: "#ad6c34", pts: ["خوف غير منطقي من فقدان الأشخاص الذين يمثلون مساحة الأمان الأساسية للفرد.", "يترافق مع أفكار وهواجس مستمرة بأن مكروهاً سيصيب أحباءه، وكوابيس متكررة حول الانفصال.", "لا يقتصر هذا الاضطراب على مرحلة الطفولة، بل يمكن أن يمتد للبالغين، مما يؤثر بشكل عميق على استقلاليتهم وأدائهم اليومي."] },
    { n: "٦", en: "Specific Phobias", ar: "الرهاب المحدد", color: "#3a6e56", pts: ["استجابة رعب مبالغ فيها ومستمرة تجاه محفز أو موقف معين كالمرتفعات أو الحيوانات أو الطيران.", "يكون القلق غير متناسب بشكل كبير مع الخطر الفعلي الذي يمثله هذا المحفز.", "يدرك الشخص عادةً أن خوفه مفرط، لكنه يعجز عن السيطرة عليه مما يدفعه لتجنب المحفز تماماً."] },
    ...
    { n: "٨", en: "Medical / Substance-Induced", ar: "القلق الناتج عن حالات طبية أو مواد", color: "#6b4255", pts: ["قلق يظهر كنتيجة مباشرة لحالة طبية عضوية كاضطرابات الغدة الدرقية أو أمراض القلب.", "قد ينتج أيضاً بسبب تأثيرات بعض الأدوية أو المواد المنشطة، أو كجزء من أعراض الانسحاب منها في الجسم."] },
];
...
const CAUSES = [
    {
        n: "١", label: "جينية ووراثية", color: "#552269", title: "العوامل الجينية والوراثية", circleBg: "linear-gradient(135deg, #3d1f4b, #af6c86)", circleColor: "#fff", borderColor: "#493054", content: (
            <div>
                <p style={{ fontSize: "0.875rem", color: "#5d5c5d", marginBottom: 10, lineHeight: 1.7, fontWeight: "600" }}>ينتقل الاستعداد للإصابة بالقلق عبر العائلة ويولد أسباباً ونتائج منها:</p>
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                    {[
                        "نقص وراثي في النواقل العصبية المهدئة (مثل GABA) ومشاكل في تنظيم السيروتونين والنور إبينفرين، مما يجعل الدماغ حساساً وفي حالة تأهب دائم.",
                        "تصبح \"اللوزة الدماغية\" (مركز الخوف) مفرطة النشاط، بينما تضعف \"قشرة الفص الجبهي\" (المنطقة المنطقية) عن كبح هذا الخوف وتهدئته."
                    ].map((p, i, a) => (
                        <li key={i} style={{ fontSize: "0.875rem", color: "#5d5c5d", padding: "0.5rem 0", borderBottom: i < a.length - 1 ? `1px solid rgba(0,0,0,0.05)` : "none", lineHeight: 1.7, fontWeight: "600" }}>
                            <span style={{ color: "#552269", marginLeft: 8 }}>{i + 1}.</span>{p}
                        </li>
                    ))}
                </ul>
            </div>
...
const BRAIN_STEPS = [
    ...
    { n: "٣", label: "PREFRONTAL CORTEX · قشرة الفص الجبهي", color: COLORS.coral, text: "يُفترض أن تقوم قشرة الفص الجبهي بتحليل الموقف بشكل منطقي وإيقاف الإنذار إذا لم يكن هناك خطر — لكنها في اضطرابات القلق تكون أضعف من المعتاد في تهدئة اللوزة الدماغية." },
...
const TREATMENTS = [
    ...
    { icon: "💊", en: "Pharmacotherapy", title: "٢. العلاج الدوائي", color: "#5c7f94", pts: [["مضادات الاكتئاب (SSRIs و SNRIs):", "الخيار الأول لعلاج معظم اضطرابات القلق، تعمل تدريجياً على تنظيم السيروتونين والنورإبينفرين وتهدئة دائرة الخوف. يُنصح بالاستمرار عليها ٦–١٢ شهراً بعد التحسن."], ["البنزوديازيبينات:", "مهدئات سريعة لنوبات الهلع الحادة، تُستخدم لفترة قصيرة جداً لتجنب التعود."], ["حاصرات بيتا:", "مثل بروبرانولول للتحكم في الأعراض الجسدية كالخفقان والارتعاش، خاصة في قلق الأداء."], ["خيارات أخرى:", "بوسبيرون لعلاج القلق العام، وهيدروكسيزين للتهدئة المؤقتة، وبريجابالين في بعض الحالات."]] },
